generate_W <- function(data, tab_ext){
  ...
  for(k in 1:nrow(XY_change)){
    ...
    if (n_select > 0) {
      WYX_new[(1+((k-1)*3)):(3+((k-1)*3)), "freq"] <- 
        data[data$X == X_select & data$Y == Y_select & data$Z == 1, 5] +
        rmultinom(n = 1, size = n_select, 
                  prob = data[data$X == X_select & data$Y == Y_select & data$Z == 0, 5]/
                    sum(data[data$X == X_select & data$Y == Y_select & data$Z == 0, 5]))
    }
    ...
    # initial sample. 
      ...
      WYX_new[(1+((k-1)*3)):(3+((k-1)*3)), "freq"] <- 
        data[data$X == X_select & data$Y == Y_select & data$Z == 1, 5] -
        rmultinom(n = 1, size = abs(n_select),
                  prob = data[data$X == X_select & data$Y == Y_select & data$Z == 1, 5]/ 
  ...
  # convert the distribution of X, Y and W to a data frame and return
